  ...
  <div class="container mx-auto px-4 max-w-6xl py-8">
    <div class="bg-slate-600 rounded-lg border border-slate-500 overflow-hidden">
      <div class="bg-slate-500 border-b border-slate-400 p-6">
        ...
        <p class="text-gray-200 mt-1 text-sm">Completa el formulario para agregar un nuevo Pokémon</p>
      </div>

      <div class="p-6">
        <form action="/pokemons/store" method="POST">
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <input 
                    ...
                    name="name" 
                    required 
                    class="w-full px-4 py-3 bg-slate-500 border border-slate-400 rounded-lg text-gray-100 placeholder-gray-300 focus:outline-none focus:border-purple-500" 
                ...
                <div>
                  ...
                  <select name="type" class="w-full px-4 py-3 bg-slate-500 border border-slate-400 rounded-lg text-gray-100 focus:outline-none focus:border-purple-500" required>
                    <option value="" disabled selected class="bg-slate-600">Select a type</option>
                    <?php foreach ($types as $type => $class): ?>
                      <option value="<?= $type ?>" class="bg-slate-600"><?= $type ?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select name="trainer_id" class="w-full px-4 py-3 bg-slate-500 border border-slate-400 rounded-lg text-gray-100 focus:outline-none focus:border-purple-500" required>
                    <option value="" disabled selected class="bg-slate-600">Select a trainer</option>
                    <?php foreach ($trainers as $t): ?>
                      <option value="<?= $t['id'] ?>" class="bg-slate-600"><?= htmlspecialchars($t['name']) ?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
              </div>
              ...
              <div class="space-y-5">
                <div class="bg-slate-500 border border-slate-400 rounded-lg p-5 space-y-3">
                  <div class="flex items-center justify-between">
                    ...
                    <span class="badge bg-cyan-500 text-white text-base px-3 py-1 rounded-full">1</span>
                  </div>
                  <input 
                    type="range" 
                    name="strength" 
          ...
          <div class="flex flex-col sm:flex-row justify-between gap-4 pt-6 mt-6 border-t border-slate-500">
            ...
            <button type="submit" class="btn bg-green-600 border-none text-white btn-lg order-1 sm:order-2">
