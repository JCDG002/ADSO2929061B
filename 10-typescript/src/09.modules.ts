...
let teslaCompanies : number = Number(prompt('Carlos, ¿cuántas empresas de Tesla admiras? (ej: 3): ') || '3');
let spacexProjects : number = Number(prompt('¿Cuántos proyectos de SpaceX te interesan? (ej: 5): ') || '5');
let totalInterest : number = sumar(teslaCompanies, spacexProjects);
if(output9){
